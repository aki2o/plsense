...
ライブラリの検索の対象には、インストールモジュールと、プロジェクト固有モジュールがあります。

### インストールモジュール

@INCの値に基づいて検索され、どのロケーションからでも参照可能であると判断します。  
つまり、いつでも補完候補となります。

### プロジェクト固有モジュール

上記のプロジェクト固有設定ファイルのlib-pathに基づいて検索され、
同じプロジェクト内のロケーションからのみ参照可能であると判断します。  
PERL5LIBなどの環境変数、`use lib ...;`による指定を解釈することは現在のところできません。
